...
export default function Signup() {
    ...
    const validateField = (name: string, value: string) => {
        switch (name) {
            case 'firstName':
            case 'lastName':
                // Name cannot contain numbers and must be alphabetic
                if (/\d/.test(value)) {
                    return `${name.charAt(0).toUpperCase() + name.slice(1)} cannot contain numbers`
                }
                // Name should be at least 2 characters and only letters
                if (!/^[A-Za-z\s]+$/.test(value)) {
                    return `${name.charAt(0).toUpperCase() + name.slice(1)} must contain only letters`
                }
            ...
            case 'email':
                ...
                const emailRegex = /^[^\s@]+@(gmail\.com|yahoo\.com|yahoo\.in|[^\s@]+\.in)$/i
                if (!emailRegex.test(value)) {
                    return 'Email must be from .com, .yahoo.com, .yahoo.in, or .in domains'
                }
                return ''
