...
export default function Signup() {
    ...
    const validateField = (name: string, value: string) => {
        switch (name) {
            ...
            case 'subject':
                // Subject cannot be empty and cannot contain numbers
                if (/\d/.test(value)) {
                    return 'Subject cannot contain numbers'
                }
                if (value.trim().length < 2) {
            ...
            case 'email':
                ...
                const emailRegex = /^[^\s@]+@[^\s@]+\.(com|yahoo|in)$/i
                if (!emailRegex.test(value)) {
                    return 'Email must end with .com, .yahoo, or .in'
                }
                return ''
                ...
                <div className="flex flex-col gap-6 w-full md:w-1/2 max-w-md">
                    ...
                    <div>
                        <input 
                            ...
                            onChange={handleChange}
                            required
                            className={`w-full px-4 py-3 text-black rounded-lg border ${errors.subject ? 'border-red-500' : 'border-gray-300'} focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all duration-200 bg-gray-50 hover:bg-gray-100`}
                        />
                        {errors.subject && (
